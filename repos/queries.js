...
const METER_READING_INSERT_QUERY = `
    INSERT INTO meter_readings (reading_date, import_units, export_units, days_span, import_per_day, export_per_day,
...
const YEAR_TOTAL_UNITS_QUERY = (year) => `
    SELECT ROUND(SUM(units_produced), 2)
    FROM solar_day_stats
    where day BETWEEN '${year}-01-01 00:00:00' AND '${year}-12-31 23:59:59'
`;
...
const FETCH_METER_READINGS_QUERY = (reading_date) => {
    const SELECT = `SELECT *
                    FROM meter_readings`;
    const DATE_CLAUSE = ` reading_date < $1`;
    const ORDER_LIMIT = ` ORDER BY reading_date DESC LIMIT 20`;

    if (reading_date)
        return `${SELECT} WHERE ${DATE_CLAUSE} ${ORDER_LIMIT}`;
    else {
        return `${SELECT} ${ORDER_LIMIT}`;
    }
}

const FETCH_USAGE_STATS_QUERY = (lastId) => {
    const SELECT = `SELECT *
                    FROM usage_stats`;
    const DATE_CLAUSE = ` id < $1`;
    const ORDER_LIMIT = ` ORDER BY id DESC LIMIT 20`;
...
const FETCH_YEAR_STATS_QUERY = `SELECT *
                                FROM solar_year_stats
                                ORDER BY YEAR DESC`;

const FETCH_STATS_OF_DAY_QUERY = `SELECT SUM(units_produced) as total_production
                                  FROM solar_day_stats
                                  where day <= $1`;
